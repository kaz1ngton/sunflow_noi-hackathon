...
def run_pre_process(dataset_path) -> str:
    ...
    upper_corr = dataframe.corr().where(np.triu(np.ones(dataframe.corr().shape), k=1).astype(bool))
    ...
    new_dataframe = dataframe[:dataframe['Year'].diff()[dataframe['Year'].diff() < 0].index[0]]
    print(f"Dropped {dataframe.shape[0] - new_dataframe.shape[0]} rows as data is non-consequent")
    dataframe = new_dataframe

    # Parse time
    print("Parsing timestmap")
    dataframe['Timestamp'] = (dataframe['Timestamp'].dt.hour * 3600 + dataframe['Timestamp'].dt.minute * 60 + dataframe['Timestamp'].dt.second).astype('int')
...
def run_model_train(dataframe): #TODO: CHANGE
    ...
    y = dataframe['Power (W)']
    training_set, validation_set = train_test_split(dataframe, test_size = 0.2, random_state = 21)
